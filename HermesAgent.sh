...
show_first_install_instructions(){
  echo
  echo -e "${CYAN}Сейчас откроется официальный мастер Hermes.${NC}"
  echo "Для нашей сборки:"
  echo
  ...
  echo
  echo "Node/browser dependencies официального installer мы пропускаем:"
  echo "для Browser дальше ставится отдельный Camofox."
  echo
...
install_hermes(){
  ...
  if find_hermes; then
    ok "Hermes уже установлен: $(hermes --version 2>/dev/null || echo installed)"
    return 0
  fi
  ...
  local tmp
  tmp="$(mktemp)"
  info "Устанавливаю Hermes без ненужного npm/browser bootstrap..."
  curl -fsSL -H 'Cache-Control: no-cache, no-store' \
    "$(cache_url "$BASE_URL/HermesCoreInstall.sh")" -o "$tmp"
  chmod +x "$tmp"
  bash "$tmp"
  rm -f "$tmp"
  runtime_path

  find_hermes || { err "Hermes не найден после установки."; return 1; }
...
install_active_soul(){
  ...
  if [ "$FRESH_INSTALL" = "1" ]; then
    ...
    warn "Ваш SOUL.md не тронут; новый пример сохранён рядом."
  fi
  rm -f "$tmp"
}
...
browser_camofox_setup(){
  ...
  local tmp
  tmp="$(mktemp)"
  info "Устанавливаю / проверяю Camofox anti-detection Browser..."
...
1. Настроить Telegram через пункт 4.
2. Включить Gateway 24/7 через пункт 5.
3. Настроить Tools через пункт 6.
4. Если нужен X Search — пункт 7.
5. GitHub: gh auth login
6. Signal Radar / Evidence Dive запускайте уже через Telegram.
7. Когда ручной результат устраивает — включайте Cron.
...
tools(){
  ...
  hermes tools
  echo
  info "Tools сохранены. Возвращаю Browser на Camofox..."
  browser_camofox_setup
}
...
diag(){
  ...
  if find_hermes; then
    ...
    if docker ps --format '{{.Names}}' | grep -qx 'camofox-browser' && \
       curl -fsS http://127.0.0.1:9377/health >/dev/null 2>&1; then
